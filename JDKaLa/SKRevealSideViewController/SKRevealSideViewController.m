...
@interface SKRevealSideViewController (Public)
...
@implementation SKRevealSideViewController
...
- (id) initWithRootViewController:(UIViewController*)rootViewController {
    ...
    if (self)
    {
        ...
        self.directionsToShowBounce = PPRevealSideDirectionLeft;
...
{
    [self informDelegateWithOptionalSelector:@selector(skrevealSideViewController:willPushController:) withParam:controller];
                             ...
                             if ([self canCrossOffsets]) // then we come to normal
                             {
                                 [UIView animateWithDuration:OpenAnimationTime*OpenAnimationTimeBouncingRatio
                                                  ...
                                                  animations:^{
                                                      ...
                                                      [self informDelegateWithOptionalSelector:@selector(skrevealSideViewController:didPushController:) withParam:controller];
                                                  }];
                             }
                             else
                             {
...
- (void) popViewControllerWithNewCenterController:(UIViewController *)centerController animated:(BOOL)animated andPresentNewController:(UIViewController *)controllerToPush withDirection:(PPRevealSideDirection)direction andOffset:(CGFloat)offset {
    ...
    [self informDelegateWithOptionalSelector:@selector(skrevealSideViewController:willPopToController:) withParam:centerController];
    ...
    void (^bigAnimBlock)(BOOL) = ^(BOOL finished) {
        if (finished) {
            ...
            void (^smallAnimBlockCompletion)(BOOL) = ^(BOOL finished) {
                if (finished) {
                    [self informDelegateWithOptionalSelector:@selector(skrevealSideViewController:didPopToController:) withParam:centerController];
...
#pragma clang diagnostic pop
    ...
    if (selector == @selector(skrevealSideViewController:didPushController:)
        ||
        selector == @selector(skrevealSideViewController:didPopToController:)) {
        [self addGesturesToCenterController];
    }
}
...
- (UIViewController*) getControllerForGestures
{
    ...
    if ([self.delegate respondsToSelector:@selector(controllerForGesturesOnSKRevealSideViewController:)]) {
        UIViewController *specialController = [self.delegate controllerForGesturesOnSKRevealSideViewController:self];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if ([self.delegate respondsToSelector:@selector(skrevealSideViewController:shouldDeactivateGesture:forView:)])
        hasExceptionDelegate = [self.delegate skrevealSideViewController:self
                                                 shouldDeactivateGesture:gestureRecognizer
                                                                 forView:touch.view];
    
    if ([self.delegate respondsToSelector:@selector(skrevealSideViewController:directionsAllowedForPanningOnView:)]) {
        _disabledPanGestureDirection = [self.delegate skrevealSideViewController:self directionsAllowedForPanningOnView:touch.view];
    }
    else
...
@implementation UIViewController (SKRevealSideViewController)
static char revealSideViewControllerKey;

- (void) setRevealSideViewController:(SKRevealSideViewController *)revealSideViewController {
...
- (SKRevealSideViewController*) revealSideViewController {
...
@implementation UIView (SKRevealSideViewController)
