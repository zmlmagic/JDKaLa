...
@protocol SKRevealSideViewControllerDelegate;
...
/** Allow pushing controllers on side views.
 ...
 _revealSideViewController = [[SKRevealSideViewController alloc] initWithRootViewController:nav];
...
@interface SKRevealSideViewController : UIViewController <UIGestureRecognizerDelegate>
{
...
/**
 The delegate which will receive events from the controller. See SKRevealSideViewControllerDelegate for more informations.
 */
@property (nonatomic, assign) id <SKRevealSideViewControllerDelegate> delegate;
...
/**
 Update the view with gestures. Should be called for example when used with controllerForGesturesOnSKRevealSideViewController delegate method when using a container controller as the root. For example with a UITabBarController, call this method when the selected controller has been updated
 */
- (void) updateViewWhichHandleGestures;
@end

/**
 UIViewController category for the SKRevealSideViewController
 */
@interface UIViewController (SKRevealSideViewController)
/**
 The parent revealSideViewController
 */
@property (nonatomic, retain) SKRevealSideViewController *revealSideViewController;
@end

/**
 UIView category to add a content inset when you push a view with an offset
 */
@interface UIView (SKRevealSideViewController)
/**
...
/**
 SKRevealSideViewControllerDelegate protocol
 */
@protocol SKRevealSideViewControllerDelegate <NSObject>
@optional
 ...
 */
- (void) skrevealSideViewController:(SKRevealSideViewController *)controller didChangeCenterController:(UIViewController*)newCenterController;

/** Called when a controller will be pushed
 @param controller The reveal side view controller
 @param pushedController The controller pushed
 */
- (void) skrevealSideViewController:(SKRevealSideViewController *)controller willPushController:(UIViewController *)pushedController;

/** Called when a controller has been pushed
 @param controller The reveal side view controller
 @param pushedController The controller pushed
 */
- (void) skrevealSideViewController:(SKRevealSideViewController *)controller didPushController:(UIViewController *)pushedController;

/** Called when a controller will be poped
 @param controller The reveal side view controller
 @param centerController The center controller poped
 */
- (void) skrevealSideViewController:(SKRevealSideViewController *)controller willPopToController:(UIViewController *)centerController;

/** Called when a controller has been poped
 @param controller The reveal side view controller
 @param centerController The center controller poped
 */
- (void) skrevealSideViewController:(SKRevealSideViewController *)controller didPopToController:(UIViewController *)centerController;
...
/** Called when a gesture will start. Typically, if you would use a class like the UISlider (handled by default in the class), you don't want to activate the pan gesture on the slider since it will not be functional.
 ...
 @see skrevealSideViewController:directionsAllowedForPanningOnView:
 
 */
- (BOOL) skrevealSideViewController:(SKRevealSideViewController *)controller shouldDeactivateGesture:(UIGestureRecognizer*)gesture forView:(UIView*)view;
...
/**
 ...
 - (PPRevealSideDirection)skrevealSideViewController:(SKRevealSideViewController*)controller directionsAllowedForPanningOnView:(UIView*)view {
 ...
 @see skrevealSideViewController:shouldDeactivateGesture:forView:
 */
- (PPRevealSideDirection)skrevealSideViewController:(SKRevealSideViewController*)controller directionsAllowedForPanningOnView:(UIView*)view;
 ...
 */
- (UIViewController*) controllerForGesturesOnSKRevealSideViewController:(SKRevealSideViewController*)controller;
@end
