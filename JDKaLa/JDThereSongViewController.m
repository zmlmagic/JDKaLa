...
@interface JDThereSongViewController()

@end

@implementation JDThereSongViewController
...
- (id)initWithTitleFileName:(NSString *)_string
{
    ...
    if(self)
    {
        ...
        JDThemeXMLPaser *paser = [[JDThemeXMLPaser alloc] initWithFileName:_string];
        [self performSelectorInBackground:@selector(installTableViewInBack:) withObject:paser.model_theme.array_song];
        self.model_theme = paser.model_theme;
        [UIUtils view_showProgressHUD:@"正在努力加载中..." forWaitInView:self.view];
...
- (void)installTableViewInBack:(NSMutableArray *)songArray
{
    JDSqlDataBase   *dataController = [[JDSqlDataBase alloc] init];
    self.array_data = [dataController selectSongArrayWithArray:songArray];
    [dataController release];
    [self configureTable_data];
    [self configureView_table];
    [UIUtils view_hideProgressHUDinView:self.view];
    
    [self performSelectorOnMainThread:@selector(configureTable_detailView) withObject:nil waitUntilDone:YES];
}
...
- (void)configureView_title:(NSString *)string_title
{
    ...
    if(IOS7_VERSION)
    {
        UIView *view_state = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 1024, 20)];
        [view_state setBackgroundColor:[UIColor blackColor]];
        [self.view addSubview:view_state];
        [view_state release];
    }
}
...
- (void)configureTable_detailView
{
    UIView *view_detail = [[UIView alloc] initWithFrame:CGRectMake(0, 50, 1024, 267)];
    IOS7(view_detail);
    //[view_detail setBackgroundColor:[UIColor redColor]];
    [self.view addSubview:view_detail];
    [view_detail release];

    _view_title = view_detail;
    
    UISwipeGestureRecognizer *recognizer_up = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipeFrom_up)];
    recognizer_up.direction = UISwipeGestureRecognizerDirectionUp;
    [view_detail addGestureRecognizer:recognizer_up];
    [recognizer_up release];
    
    UIImageView *imageView_theme = [[UIImageView alloc] initWithFrame:CGRectMake(25, 25, 175, 175)];
    [imageView_theme setImageWithURL:[NSURL URLWithString:_model_theme.string_imageTheme]];
    [view_detail addSubview:imageView_theme];
    [imageView_theme release];
    
    UIImageView *imageView_theme_back = [[UIImageView alloc] initWithFrame:CGRectMake(25, 25, 180, 180)];
    [UIUtils didLoadImageNotCached:@"singerBack_detail.png" inImageView:imageView_theme_back];
    [view_detail addSubview:imageView_theme_back];
    [imageView_theme_back release];
    
    UITextView *textDetail = [[UITextView alloc] initWithFrame:CGRectMake(220, 30, 750, 185)];
    [textDetail setFont:[UIFont systemFontOfSize:18.0f]];
    [textDetail setBackgroundColor:[UIColor clearColor]];
    [textDetail setTextColor:[UIColor blackColor]];
    [textDetail setText:[NSString stringWithFormat:@"       %@",_model_theme.string_themeDetail]];
    [view_detail addSubview:textDetail];
    [textDetail release];
    
    UIImageView *image_songList = [[UIImageView alloc] initWithFrame:CGRectMake(30, 246, 964, 15)];
    [UIUtils didLoadImageNotCached:@"songList.png" inImageView:image_songList];
    [view_detail addSubview:image_songList];
    [image_songList release];
    
    [self configureView_title:_model_theme.string_title];
}

- (void)handleSwipeFrom_up
{
    [_table_song setFrame:CGRectMake(_table_song.frame.origin.x, _table_song.frame.origin.y, _table_song.frame.size.width, _table_song.frame.size.height + 240)];
    
    [UIUtils removeViewWithAnimation:_view_title inCenterPoint:CGPointMake(_view_title.center.x, _view_title.center.y - 240) withBoolRemoveView:NO];
    
    [self performSelector:@selector(handleSwipeFrom_up_songView) withObject:nil afterDelay:0.1];
    
    _bool_up = YES;
    //NSLog(@"swiper");
}

- (void)handleSwipeFrom_up_songView
{
    [UIUtils removeViewWithAnimation:_table_song inCenterPoint:CGPointMake(_table_song.center.x, _table_song.center.y - 240) withBoolRemoveView:NO];
}
...
{
    //scrollView.
    if(scrollView.contentOffset.y <0 && _bool_up)
    {
        [UIUtils removeViewWithAnimation:_view_title inCenterPoint:CGPointMake(_view_title.center.x, _view_title.center.y + 240) withBoolRemoveView:NO];
        
        [UIUtils removeViewWithAnimation:_table_song inCenterPoint:CGPointMake(_table_song.center.x, _table_song.center.y + 240) withBoolRemoveView:NO];
        //[_table_song setFrame:CGRectMake(_table_song.frame.origin.x, _table_song.frame.origin.y, _table_song.frame.size.width, _table_song.frame.size.height - 240)];
        [self performSelector:@selector(didMoveToUpSetTabelFrame) withObject:nil afterDelay:0.5f];
...
{
    UITableView *table_singer = [[UITableView alloc] initWithFrame:CGRectMake(0, 50+267, 1024, 691-267) style:UITableViewStylePlain];
    IOS7(table_singer);
