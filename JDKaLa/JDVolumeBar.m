...
/**
 旋转中心
 **/
#define CIRCLE_X                        (66.0f)
#define CIRCLE_Y                        (66.0f)

/**起始和结束角度**/
#define START_ANGLE                     (-50.0f)
#define END_ANGLE                       (230.0f)
/**
 控制点距离中心距离
 **/
#define CONTROL_CIRCLE_RADIUS           (25.0f)
/**
 旋转半径
 **/
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        [self setUserInteractionEnabled:YES];
        _backgroundView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"circle_big_bg.png"]];
        ...
        //control
        _circleSlide = \
        [[CircleSlide alloc] initWithImage:[UIImage imageNamed:@"vol_ctrl1.png"]
        ...
        //content
        _contentImage = [UIImage imageNamed:@"circle_big_color.png"];
...
{
    CGContextRef context = CGBitmapContextCreate(NULL, self.bounds.size.width, self.bounds.size.height, 8, 4 * self.bounds.size.width, CGColorSpaceCreateDeviceRGB(), 4);
